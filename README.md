<div align="center">

### 👋 Hi, I'm birdger

🔬 研究兴趣：**多模态推荐系统** · **大模型增强推荐（LLM4Rec）** · **跨模态融合**
🧪 在读研究生，方向 AI / 推荐系统

</div>

---

### 📡 每日自动更新的科研雷达

[**daily-arxiv-digest**](https://github.com/birdger/daily-arxiv-digest) —— 每天自动抓取 arXiv 的 `cs.IR` / `cs.CL` / `cs.CV` / `cs.MM` / `cs.LG`，
按关键词加权打分，生成一份中文论文日报并自动提交。零成本跑在 GitHub Actions 上。

[![Daily arXiv Digest](https://github.com/birdger/daily-arxiv-digest/actions/workflows/daily.yml/badge.svg)](https://github.com/birdger/daily-arxiv-digest/actions/workflows/daily.yml)

### 🧪 正在做的事

- 复现 **生成式推荐 / 序列推荐 / 多模态融合** 方向的代表性工作
- 探索 **LLM 富化 + 多模态融合** 在推荐系统上的实证效果
- 维护上面那份自动更新文献池

### 🛠 技术栈

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![pandas](https://img.shields.io/badge/pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![LaTeX](https://img.shields.io/badge/LaTeX-008080?style=flat-square&logo=latex&logoColor=white)

### 📊 GitHub

<div align="center">

![GitHub 概览](assets/stats.svg)

![Followers](https://img.shields.io/github/followers/birdger?style=flat-square&color=82aaff&label=Followers)
![Stars](https://img.shields.io/github/stars/birdger/daily-arxiv-digest?style=flat-square&color=82aaff&label=arXiv%20Radar%20Stars)
![Last commit](https://img.shields.io/github/last-commit/birdger/daily-arxiv-digest?style=flat-square&color=82aaff&label=%E6%9C%80%E8%BF%91%E6%8F%90%E4%BA%A4)

<img src="https://streak-stats.demolab.com/?user=birdger&theme=tokyonight&hide_border=true" />

<sub>上方卡片由本仓库的 GitHub Actions 每周自动生成并提交，不依赖第三方统计服务。</sub>

</div>

---

<div align="center">
<sub>由 <a href="https://github.com/birdger/daily-arxiv-digest">daily-arxiv-digest</a> 的研究节奏驱动 · 每天进步一点点</sub>
</div>
